...
// Fix: Reusable multi-term report component
const MultiTermReportCard: React.FC<{
    ...
    const finalRemark = useMemo(() => {
        ...
        if (summary3?.result === 'PASS' || summary3?.result === 'SIMPLE PASS') {
            ...
            if (nextGrade) {
                return `Promoted to ${nextGrade}. School reopens on April 1, 2026`;
            }
            return `Promoted. School reopens on April 1, 2026`;
        } else if (summary3?.result === 'FAIL') {
            return "Detained";
        }
        return summaries.terminal3?.remark || "Awaiting final results.";
    }, [summaries.terminal3, student.grade]);
...
// Fix: Reusable single-term report card component
...
// Fix: Bulk progress report page logic cleaned of corrupted markers
