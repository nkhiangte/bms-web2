...
const calculateTermSummary = (
    ...
    const studentData = allStudents.map(s => {
        ...
        // Using for...of loop for improved type narrowing and cleaner arithmetic
        for (const sd of numericSubjects) {
            ...
            if (hasActivities) {
                ...
                // Arithmetic addition is safe when both operands are confirmed numbers
  ...
  const processedData: ProcessedStudent[] = useMemo(() => {
    ...
    const studentData = classStudents.map(student => {
      ...
      for (const sd of numericSubjects) {
        ...
        // Fixed arithmetic to avoid type inference issues
      ...
      const percentageValue = fullMarksTotal > 0 ? (grandTotal / fullMarksTotal) * 100 : 0;
      ...
      let divisionValue = isClassIXorX && resultValue === 'PASS' ? (percentageValue >= 75 ? 'Distinction' : percentageValue >= 60 ? 'I Div' : percentageValue >= 45 ? 'II Div' : percentageValue >= 35 ? 'III Div' : '-') : '-';
      let academicGradeValue = resultValue === 'FAIL' ? 'E' : (percentageValue > 89 ? 'O' : percentageValue > 79 ? 'A' : percentageValue > 69 ? 'B' : percentageValue > 59 ? 'C' : 'D');
      
      let remarkText = '';
      if (resultValue === 'FAIL') {
          remarkText = `Needs improvement in ${failedSubjectsList.join(', ')}`;
      } else if (resultValue === 'SIMPLE PASS') {
          remarkText = `Focus on ${failedSubjectsList.join(', ')}`;
      } else {
          if (percentageValue >= 90) remarkText = "Outstanding performance!";
          else if (percentageValue >= 75) remarkText = "Excellent progress. Keep up the great work.";
          else if (percentageValue >= 60) remarkText = "Good progress. Well done.";
          else if (percentageValue >= 45) remarkText = "Satisfactory performance. Consistent effort will lead to better results.";
          else remarkText = "Passed. Consistent effort is needed to improve scores.";
      }
      ...
      return { 
          ...
          academicGrade: academicGradeValue, 
          remark: remarkText 
        };
    });
