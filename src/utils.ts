import { Student, Staff, Grade, FeePayments, GradeDefinition, StaffAttendanceRecord, StudentAttendanceRecord, AttendanceStatus, StudentAttendanceStatus, FeeStructure, HostelDisciplineEntry, HostelResident } from './types';
...
export const formatStudentId = (student: Student, academicYear: string): string => {
    ...
    // A student must have a grade and roll number to generate an ID.
    // academicYear must also be present.
    if (!student?.grade || typeof student.rollNo !== 'number' || !academicYear) {
        // Fallback to a non-crashing value. The database ID is a good unique identifier.
...
export const formatDateForNews = (isoDate?: string): string => {
    if (!isoDate) return '';
    // Add T00:00:00 to ensure the date is parsed in the local timezone, avoiding off-by-one day errors.
...
export const getHolidayDates = (events: any[]): Set<string> => {
    const holidayDates = new Set<string>();
    if (!events) return holidayDates;
...
export const exportDisciplineLogToCsv = ({
    ...
    const rows = logEntries.map(entry => {
        ...
        const student = resident ? studentMap.get(resident.studentId) : undefined;
...
export const getCurrentAcademicYear = (): string => {
    ...
    // The academic year typically starts in April (month 3).
    // If the current month is January, February, or March (0, 1, 2),
    // we are still in the academic year that started in the previous calendar year.
...
export const uploadToImgBB = async (base64Image: string): Promise<string> => {
    ...
    // Remove the data:image/jpeg;base64, prefix if present to send just the base64 data
    // ImgBB accepts base64 strings as the 'image' parameter.
