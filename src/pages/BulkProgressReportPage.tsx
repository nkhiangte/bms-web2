...
const calculateTermSummary = (
    ...
    if (exam?.results) {
        exam.results.forEach(res => {
            // FIX: Use subjectsMatch (alias-aware) instead of plain map key lookup
            // This prevents "Social Science" and "Social Studies" from becoming two separate rows
            const alreadyExists = Array.from(subjectsMap.keys()).some(existingKey =>
                subjectsMatch(existingKey, res.subject)
            );
            if (!alreadyExists) {
                subjectsMap.set(normalizeSubjectName(res.subject), {
                    name: res.subject,
                    examFullMarks: 100,
                    activityFullMarks: 0,
