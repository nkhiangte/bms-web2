  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route path="/signup" element={<SignUpPage onSignUp={async (n, e, p) => { try { const c = await auth.createUserWithEmailAndPassword(e, p); if (c.user) { await c.user.updateProfile({ displayName: n }); await db.collection('users').doc(c.user.uid).set({ displayName: n, email: e, role: 'pending', createdAt: new Date().toISOString(), registeredAt: new Date().toISOString() }); return { success: true, message: 'Awaiting approval.' }; } return { success: false }; } catch (err: any) { return { success: false, message: err.message }; } }} />} />
