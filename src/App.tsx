...
const App: React.FC = () => {
      ...
      try {
          ...
          if (staffId) {
              ...
              if (doc.exists) {
                  ...
                  if (assignedGradeKey) {
                      if (!currentDefs[assignedGradeKey]) currentDefs[assignedGradeKey] = { subjects: [] };
          ...
          addNotification('Failed to save staff member: ' + error.message, 'error');
          throw error;
      }
  };
  ...
  const handleDeleteStaff = async (id: string) => {
      try {
          ...
          // Also remove from any class teacher role
          const gradeDefRef = db.collection('config').doc('gradeDefinitions');
          const doc = await gradeDefRef.get();
          if (doc.exists) {
              const currentDefs = doc.data() as Record<Grade, GradeDefinition>;
          ...
          addNotification('Failed to delete staff member: ' + error.message, 'error');
          throw error;
      }
  };
