...
const MultiTermReportCard: React.FC<{
  ...
  const isIXTerminal3Report =
    student.grade === Grade.IX || student.grade === Grade.X;
  ...
  const getAttendancePercent = (attendance?: Attendance) => {
    if (attendance && attendance.totalWorkingDays > 0) {
      return `${(
        (attendance.daysPresent / attendance.totalWorkingDays) *
        100
      ).toFixed(0)}%`;
    }
    return "-";
  ...
  const finalRemark = useMemo(() => {
    ...
    const nextGradeLabel = nextGrade
      ? gradeLabel[nextGrade] ?? nextGrade
      : null;
    ...
    return (
      exam3?.teacherRemarks || summary3?.remark || "Awaiting final results."
    );
  ...
  return (
    <div>
      ...
      >
        <thead>
          <tr style={{ background: "#f1f5f9" }}>
            <th rowSpan={hasActivities || isIXorX ? 2 : 1} style={thStyle}>
              SUBJECT
            </th>
            <th colSpan={isIXorX ? 2 : hasActivities ? 2 : 1} style={thStyle}>
              I Terminal
            </th>
            <th colSpan={isIXorX ? 2 : hasActivities ? 2 : 1} style={thStyle}>
              II Terminal
              ...
              <tr key={sd.name} style={{ textAlign: "center" }}>
                ...
                {hasActivities ? (
                  isGraded ? (
                    ...
                    <>
                      <td style={tdStyle}>{t1?.examMarks ?? "-"}</td>
                      <td style={tdStyle}>{t1?.activityMarks ?? "-"}</td>
                      <td style={tdStyle}>{t2?.examMarks ?? "-"}</td>
                      <td style={tdStyle}>{t2?.activityMarks ?? "-"}</td>
                      <td style={tdStyle}>{t3?.examMarks ?? "-"}</td>
                      <td style={tdStyle}>{t3?.activityMarks ?? "-"}</td>
                    </>
                  )
                ) : isIXTerminal3Report ? (
                  isGraded ? (
                    ...
                    <>
                      ...
                      <td style={tdStyle}>{t3?.saMarks ?? t3?.marks ?? "-"}</td>
                      <td style={tdStyle}>{t3?.faMarks ?? "-"}</td>
                    </>
                  )
                ) : (
                  <>
                    <td style={tdStyle}>
                      {isGraded ? t1?.grade ?? "-" : t1?.marks ?? "-"}
                    </td>
                    <td style={tdStyle}>
                      {isGraded ? t2?.grade ?? "-" : t2?.marks ?? "-"}
                    </td>
                    <td style={tdStyle}>
                      {isGraded ? t3?.grade ?? "-" : t3?.marks ?? "-"}
                    </td>
                  </>
                )}
              </tr>
        ...
        <tfoot>
          {hasActivities && (
            <tr style={{ fontWeight: "bold", textAlign: "center" }}>
              ...
              <td style={tdStyle}>{summaries.terminal1?.examTotal ?? "-"}</td>
              <td style={tdStyle}>
                {summaries.terminal1?.activityTotal ?? "-"}
              </td>
              <td style={tdStyle}>{summaries.terminal2?.examTotal ?? "-"}</td>
              <td style={tdStyle}>
                {summaries.terminal2?.activityTotal ?? "-"}
              </td>
              <td style={tdStyle}>{summaries.terminal3?.examTotal ?? "-"}</td>
              <td style={tdStyle}>
                {summaries.terminal3?.activityTotal ?? "-"}
              </td>
            </tr>
          )}
          {[
            [
              "Grand Total",
              summaries.terminal1?.grandTotal,
              summaries.terminal2?.grandTotal,
              summaries.terminal3?.grandTotal,
            ],
            [
              "Result",
              summaries.terminal1?.result,
              summaries.terminal2?.result,
              summaries.terminal3?.result,
            ],
            [
              "Rank",
              summaries.terminal1?.rank,
              summaries.terminal2?.rank,
              summaries.terminal3?.rank,
            ],
            [
              "Percentage",
              summaries.terminal1?.percentage != null
                ? summaries.terminal1.percentage.toFixed(1)
                : "-",
              summaries.terminal2?.percentage != null
                ? summaries.terminal2.percentage.toFixed(1)
                : "-",
              summaries.terminal3?.percentage != null
            ...
            [
              ...
              isIXorX
                ? summaries.terminal1?.division
                : summaries.terminal1?.academicGrade,
              isIXorX
                ? summaries.terminal2?.division
                : summaries.terminal2?.academicGrade,
              isIXorX
                ? summaries.terminal3?.division
                : summaries.terminal3?.academicGrade,
            ],
            [
              "Attendance %",
              getAttendancePercent(exams.terminal1?.attendance),
              getAttendancePercent(exams.terminal2?.attendance),
              getAttendancePercent(exams.terminal3?.attendance),
            ],
          ].map(([label, v1, v2, v3]) => (
            <tr
              key={label as string}
            ...
            >
              ...
              <td colSpan={hasActivities || isIXorX ? 2 : 1} style={tdStyle}>
                {v1 ?? "-"}
              </td>
              <td colSpan={hasActivities || isIXorX ? 2 : 1} style={tdStyle}>
                {v2 ?? "-"}
      ...
      <div style={{ marginTop: "1.5in", fontSize: "8pt" }}>
        <div
          style={{
            display: "flex",
    ...
    <div>
              ...
              <tr key={sd.name}>
                ...
                {isNurseryToII ? (
                  <>
                    ...
                    <td style={tdStyle}>{isGraded ? "-" : 35}</td>
                  ...
                  isGraded ? (
                    ...
                    <>
                      <td style={tdStyle}>{sd.examFullMarks}</td>
                      <td style={tdStyle}>{result?.examMarks ?? 0}</td>
                      <td style={tdStyle}>{sd.activityFullMarks}</td>
                      <td style={tdStyle}>{result?.activityMarks ?? 0}</td>
                      <td style={{ ...tdStyle, fontWeight: "bold" }}>
                  ...
                  <>
                    ...
                    <td style={tdStyle}>{isGraded ? "-" : 33}</td>
                    <td style={{ ...tdStyle, fontWeight: "bold" }}>
                      {isGraded ? result?.grade || "-" : result?.marks ?? 0}
                    </td>
                  </>
      ...
      >
        ...
        >
          ...
          <span
            style={{ textAlign: "right", color: "#475569", fontWeight: 600 }}
          >
            Grand Total:
          </span>
          <span style={{ fontWeight: "bold" }}>
          ...
          <span
            style={{
              fontWeight: "bold",
              color:
                processedReportData?.result !== "PASS" ? "#dc2626" : "#059669",
            }}
          >
...
const ProgressReportPage: React.FC<ProgressReportPageProps> = ({
  ...
  const summaries = {
    terminal1: useMemo(
      () =>
        student && gradeDef && classmates.length > 0
  ...
  if (!gradeDef)
    return (
      <div className="p-8 text-center">
        <p>Curriculum not defined for {student.grade}.</p>
      </div>
    );
    ...
    <div className="bg-slate-100 print:bg-white">
      ...
      <div className="print-hidden container mx-auto p-4 flex justify-between items-center sticky top-0 bg-slate-100/80 backdrop-blur-sm z-10 shadow-sm">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-slate-600">
            {student.name} — {examTemplate?.name}
          </p>
        </div>
      ...
      <div className="container mx-auto bg-white p-6 my-4 shadow-lg print:hidden">
        <p className="text-xs text-slate-400 mb-2 text-center">
          Screen preview — actual print output uses A4 with 8 cm top margin for
          letterhead.
        </p>
      </div>
      ...
      {/* ── Printable area ── */}
      <div
        id="progress-report-printable"
        className="container mx-auto bg-white p-6 my-4 shadow-lg print:w-full print:max-w-none print:my-0 print:shadow-none print:p-0"
      >
        <div className="font-serif text-xs text-slate-900">
          ...
          <section className="mb-1 border-2 border-slate-400 rounded text-xs flex items-stretch">
            <div className="flex-1 p-1.5 grid grid-cols-3 gap-x-2 gap-y-0.5 content-start">
              ...
              <div>
                <strong className="block text-slate-600">Father's Name:</strong>
                <span className="font-bold">{student.fatherName}</span>
              </div>
              <div>
                <strong className="block text-slate-600">Date of Birth:</strong>
