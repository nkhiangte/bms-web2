...
const EnrollStudentModal: React.FC<EnrollStudentModalProps> = ({ isOpen, onClose, onEnroll, admission, academicYear, isProcessing }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            setTargetGrade(admission.admissionGrade as Grade);
        }
    }, [isOpen, admission]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Map OnlineAdmission fields to Student fields
        const studentData: Omit<Student, 'id'> = {
            rollNo: rollNo,
            name: admission.studentName,
            grade: targetGrade,
            studentId: '', // Will be generated by the handler/utility
            ...
            bloodGroup: admission.bloodGroup as BloodGroup,
            cwsn: admission.cwsn || 'No',
            fatherName: admission.fatherName,
            fatherOccupation: admission.fatherOccupation || '',
            fatherAadhaar: admission.parentAadhaar || '', // Best guess mapping
            motherName: admission.motherName,
            motherOccupation: admission.motherOccupation || '',
            motherAadhaar: '', // Not in admission form
            ...
            photographUrl: '', // To be updated manually later or mapped if exists
            feePayments: createDefaultFeePayments(),
            academicPerformance: [],
            academicYear: academicYear
        };

        // Generate the formatted ID based on selected roll and grade
        studentData.studentId = formatStudentId(studentData as Student, academicYear);
                    ...
                    <div className="p-6 space-y-4">
                        <div className="bg-sky-50 p-4 rounded-lg border border-sky-100 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-600 uppercase tracking-wider font-semibold">Application Ref: {admission.id}</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">Confirm Grade</label>
                                <select 
                                    value={targetGrade} 
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-1">Assign Roll Number</label>
                                <input 
                                    type="number" 
                                    min="1"
                                    value={rollNo}
                        ...
                        <div className="p-3 bg-amber-50 rounded-lg border border-amber-100 text-xs text-amber-800">
                            <strong>Note:</strong> Enrollment will create a permanent student record for the <strong>{academicYear}</strong> academic year. This application status will be updated to "Approved".
                        </div>
                    </div>
