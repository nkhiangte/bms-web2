    ...
    const studentData = classStudents.map(student => {
      ...
      for (const sd of numericSubjects) {
        ...
        if (hasActivities) {
            ...
            /* FIX: Explicitly cast operands to numbers to resolve arithmetic operation type errors on line 252 (in original source context) */
            currentSubjFMValue = (Number(sd.examFullMarks || 0) as number) + (Number(sd.activityFullMarks || 0) as number);
