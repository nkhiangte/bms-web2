...
import { FeeStructure, Student, FeePayments, NotificationType, User, FeeSet, Grade, AdmissionSettings, FeeHead } from '../../types';
...
const AdmissionFeeSummary: React.FC<{ title: string; oneTime: FeeHead[]; annual: FeeHead[] }> = ({ title, oneTime, annual }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-md">
            <h3 className="text-xl font-bold text-slate-800 mb-4 border-b-2 border-sky-500 pb-2">{title}</h3>
            <div className="space-y-4">
                <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Admission / Registration (One-Time)</h4>
                    <ul className="space-y-1">
                        {oneTime.map(h => (
                            <li key={h.id} className="flex justify-between text-sm">
                                <span className="text-slate-700">{h.name}</span>
                                <span className="font-semibold text-slate-900">₹{h.amount}</span>
                            </li>
                        ))}
                        <li className="flex justify-between pt-1 border-t font-bold text-slate-800">
                            <span>Subtotal</span>
                ...
                <div>
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Annual / Periodic Charges</h4>
                    <ul className="space-y-1">
                        {annual.map(h => (
                            <li key={h.id} className="flex justify-between text-sm">
                                <span className="text-slate-700">{h.name}</span>
                ...
                <div className="pt-4 border-t-2 border-slate-100 flex justify-between items-center">
                    <span className="text-lg font-bold text-slate-900">Total Admission Cost</span>
                    <span className="text-2xl font-black text-sky-700">₹{totalOneTime + totalAnnual}</span>
                </div>
            </div>
        </div>
    );
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-12 relative">
                    ...
                    <div className="mt-4 text-lg text-slate-600">
                         <EditableContent id="fees_subtitle" defaultContent="Review our comprehensive fee structures and manage your school dues online." type="text" user={user} />
                    </div>
                </div>

                {/* 1. Admission Fees Structure */}
                <div className="mb-20">
                    <h2 className="text-3xl font-bold text-slate-800 text-center mb-8 border-b-2 border-slate-200 pb-4">Admission Fees (Session {admissionSettings?.academicYearLabel || '2026-27'})</h2>
                ...
                {/* 2. Ongoing Academic Fees */}
                <div className="mb-20">
                    <h2 className="text-3xl font-bold text-slate-800 text-center mb-8 border-b-2 border-slate-200 pb-4">Academic & Tuition Fees</h2>
                ...
                {/* 3. Online Payment Tool */}
                <div className="max-w-4xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-2xl border border-slate-200">
                    <h2 className="text-3xl font-bold text-slate-800 text-center mb-2">Online Payment Portal</h2>
                    <p className="text-center text-slate-600 mb-8">Pay tuition fees and other charges instantly using UPI or Credit/Debit Cards.</p>
