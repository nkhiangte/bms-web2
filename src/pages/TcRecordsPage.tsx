  ...
  const filteredRecords = useMemo(() => {
    return tcRecords.filter(record => {
        ...
        return (record.nameOfStudent ?? '').toLowerCase().includes(term) || 
               ...
               (record.pen ?? '').toLowerCase().includes(term) ||
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {filteredRecords.map(record => (
              <tr key={record.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-slate-600">{record.pen || <span className="text-slate-400 italic">N/A</span>}</td>
